...
// //     try {
// //       await loginUser(username, password); // your service logic
...
// //       <img src={logo} alt="BackTrackz Logo" className="h-32 w-32 mb-6 object-contain" />
// //       <h2 className="text-2xl font-semibold text-blue-600 mb-6">Login</h2>

// //       {error && <div className="w-full max-w-sm mb-4 text-red-600 text-center">{error}</div>}
...
//       await set(ref(db, `loginchecking/${username}`), {
//         username,
//         loginTime: now,
//         logoutTime: null, // null means session is active
//       });
...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const now = Date.now();
      await set(ref(db, `loginchecking/${trimmedUsername}`), {
        username: trimmedUsername,
        loginTime: now,
        logoutTime: null,
      });
      localStorage.setItem("loggedInUser", trimmedUsername);
      navigate("/home", { state: { username: trimmedUsername } });
    } catch (err) {
      setError(err.message || "Login failed. Please try again.");
    } finally {
      setIsLoading(false);
